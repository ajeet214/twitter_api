...
class Profileclass():
    ...
    def friendsprofile(self, name, limit):
        if limit is None:
            limit = 200
        try:
            temp = []
            for firends in tweepy.Cursor(self.api.friends, screen_name=name).items(int(limit)):
                data = self._objecttodict(firends)
                temp.append(data)
            ...
            print("error>friendsprofile ::", sys.exc_info()[1])
        finally:
            pass
...
def output2(name):
    tmp = Profileclass()
    return (tmp.profilefetcher(name))

def output3(name,limit):
    tmp = Profileclass()
    return tmp.friendsprofile(name, limit)


if __name__ == '__main__':
    tmp = Profileclass()
    # print((tmp.profilefetcher("realdonaldtrump")))
    print(tmp.friendsprofile("katyperry", 20))
